...
class GeminiProvider(AIProvider):
    ...
    async def generate(self, prompt: str, **kwargs) -> Dict:
        ...
        try:
            ...
            logger.info(f"Gemini API request payload: {json.dumps(payload, indent=2)[:500]}...")
            
            response = await self.client.post(url, json=payload)
            response.raise_for_status()

            data = response.json()
            logger.info(f"Gemini API response: {json.dumps(data, indent=2)[:1000]}...")
...
class AIProviderManager:
    ...
    def _initialize_providers(self):
        ...
        # Qwen3 Coder (for coding questions - PRIMARY)
        if hasattr(settings, 'AI_SERVICES__OPEN_ROUTER_API_KEY_QWEN_CODER') and settings.AI_SERVICES__OPEN_ROUTER_API_KEY_QWEN_CODER:
            self.providers["coding"].append(
                OpenRouterProvider(settings.AI_SERVICES__OPEN_ROUTER_API_KEY_QWEN_CODER, "qwen/qwen3-coder")
            )
        
        # xAI: Grok 4 Fast (for coding questions - BACKUP)
        if hasattr(settings, 'AI_SERVICES__OPEN_ROUTER_API_KEY_XAI') and settings.AI_SERVICES__OPEN_ROUTER_API_KEY_XAI:
            self.providers["coding"].append(
                OpenRouterProvider(settings.AI_SERVICES__OPEN_ROUTER_API_KEY_XAI, "x-ai/grok-4-fast:free")
            )

        # DeepSeek V3.1 (for reasoning)
        if hasattr(settings, 'AI_SERVICES__OPEN_ROUTER_API_KEY_DEEPSEEK') and settings.AI_SERVICES__OPEN_ROUTER_API_KEY_DEEPSEEK:
            self.providers["reasoning"].append(
                OpenRouterProvider(settings.AI_SERVICES__OPEN_ROUTER_API_KEY_DEEPSEEK, "deepseek/deepseek-chat-v3.1:free")
            )
        ...
        # qwen2.5-vl-72b-instruct:free (for image text extraction and image analysis - BACKUP)
        if hasattr(settings, 'AI_SERVICES__OPEN_ROUTER_API_KEY_1') and settings.AI_SERVICES__OPEN_ROUTER_API_KEY_1:
            self.providers["image"].append(
                OpenRouterProvider(settings.AI_SERVICES__OPEN_ROUTER_API_KEY_1, "qwen/qwen2.5-vl-72b-instruct:free")
            )

        # backup api for image
        if hasattr(settings, 'AI_SERVICES__OPEN_ROUTER_API_KEY_2') and settings.AI_SERVICES__OPEN_ROUTER_API_KEY_2:
            self.providers["image"].append(
                OpenRouterProvider(settings.AI_SERVICES__OPEN_ROUTER_API_KEY_2, "qwen/qwen2.5-vl-72b-instruct:free")
            )

        # for text - Updated Gemini model name to 2.5
