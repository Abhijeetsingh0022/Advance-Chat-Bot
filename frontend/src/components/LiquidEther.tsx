...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      _onDocumentLeave: (event: MouseEvent) => void;
...
`;
    ...
    float intensity = pow(lenv, 0.4);
    vec3 outRGB = mix(bgColor.rgb, c, intensity * 1.3);
    float outA = mix(bgColor.a, 1.0, intensity);
...
`;
    ...
    class ShaderPass {
      ...
      init() {
        ...
        if (this.uniforms) {
          ...
          if (this.scene) this.scene.add(this.plane);
        }
      }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: any) {
        ...
        if (this.mouse && this.mouse.material) {
    ...
    class Simulation {
      ...
      constructor(options: any) {
        ...
        this.fbos = {
          vel_0: new THREE.WebGLRenderTarget(1, 1),
          vel_1: new THREE.WebGLRenderTarget(1, 1),
          vel_viscous0: new THREE.WebGLRenderTarget(1, 1),
          vel_viscous1: new THREE.WebGLRenderTarget(1, 1),
          div: new THREE.WebGLRenderTarget(1, 1),
          pressure_0: new THREE.WebGLRenderTarget(1, 1),
          pressure_1: new THREE.WebGLRenderTarget(1, 1)
        };
        this.init();
      }
      ...
      update() {
        ...
        if (this.advection) this.advection.update(this.options);
        if (this.externalForce) this.externalForce.update({ ...this.options, cellScale: this.cellScale });
    ...
    class WebGLManager {
      ...
      constructor(props: any) {
        ...
        this._onVisibility = this._onVisibilityChanged.bind(this);
        window.addEventListener('resize', this._resize);
        document.addEventListener('visibilitychange', this._onVisibility);
      }

      init() {
        if (!this.props.$wrapper || !Common.renderer) return;
    ...
    try {
      ...
      const io = new IntersectionObserver(
        entries => {
          ...
          // For fixed backgrounds, always consider it visible if not hidden by document
      ...
      console.error('LiquidEther initialization error:', e);
      return () => {};
    }
  }, [
    BFECC,
